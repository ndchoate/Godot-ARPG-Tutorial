...
func move_state(delta):
    ...
    if input_vector != Vector2.ZERO:
        ...
        shieldHitbox.knockback_vector = input_vector
...
#        state = ATTACK
    ...
    if Input.is_action_just_pressed("shield%s" % ID):
        ...
        var shieldCollisionShape = shieldHitbox.get_node("ShieldCollisionShape2D")
        shieldCollisionShape.set_disabled(false)
        state = SHIELD
...
func shield_state(delta):
    if Input.is_action_just_released("shield%s" % ID):
        var shieldCollisionShape = shieldHitbox.get_node("ShieldCollisionShape2D")
        shieldCollisionShape.set_disabled(true)
        state = MOVE
